...
export class FalconAlgorithm implements PostQuantumAlgorithm {
    ...
    async generateKeys(): Promise<{ privateKey: Uint8Array; publicKey: Uint8Array; }> {
        return await falcon.keyPair()
    }

    async sign(message: Uint8Array, privateKey: Uint8Array): Promise<Uint8Array> {
        return await falcon.signDetached(message, privateKey)
    }

    async verify(signature: Uint8Array, message: Uint8Array, publicKey: Uint8Array): Promise<Boolean> {
        return await falcon.verifyDetached(signature, message, publicKey)
    }
    ...
    async performSigning(noOfMessages: number, messageLengthBytes: number) {
        ...
        console.log(`Falcon Total time for ${noOfMessages} messages: ${totalSignTime} seconds`);
      
        console.log(`Falcon Average time per message: ${totalSignTime/noOfMessages} seconds`);
    }
}
